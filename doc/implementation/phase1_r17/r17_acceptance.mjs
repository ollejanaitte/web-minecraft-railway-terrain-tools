#!/usr/bin/env node
/**
 * r15_closed_loop.mjs — Phase 1-R17 normal-world switch/junction acceptance.
...
const PHASE = resolve(ROOT, 'doc/implementation/phase1_r17');
...
const RUN_ID = process.env.RUN_ID || 'r17';
...
async function main() {
  ...
  try {
    ...
    // R17: build the corrected Standard Closed Loop (from R16), then register
    // a switch spur junction on the loop's first straight.
    ...
    // Register the switch spur (10 deg divergence) on the loop.
    const spIdx = c.lines.length;
    await chat(c, 'railsys17 spur 10');
    const spurLine = await waitConsoleAfter(c, /railsys17: spur junction/, spIdx, 30000);
    log('spur: ' + spurLine);
    if (!spurLine.includes('branches=1')) {
      throw new Error('expected 1 branch in spur: ' + spurLine);
    }

    // Route THROUGH (default): vehicle stays on the loop.
    const rtIdx = c.lines.length;
    await chat(c, 'railsys17 route sw-1 through');
    const throughLine = await waitConsoleAfter(c, /railsys17: route -> THROUGH/, rtIdx, 30000);
    log('route: ' + throughLine);
    const resTIdx = c.lines.length;
    await chat(c, 'railsys17 resolve sw-1 rail-8');
    const resTLine = await waitConsoleAfter(c, /railsys17: resolve/, resTIdx, 30000);
    log('resolve: ' + resTLine);
    if (!resTLine.includes('-> rail-1')) {
      throw new Error('expected THROUGH resolve to rail-1: ' + resTLine);
    }

    // Route BRANCH: vehicle leaves the loop onto the spur.
    const rbIdx = c.lines.length;
    await chat(c, 'railsys17 route sw-1 branch 0');
    await waitConsoleAfter(c, /railsys17: route -> BRANCH/, rbIdx, 30000);
    const resBIdx = c.lines.length;
    await chat(c, 'railsys17 resolve sw-1 rail-8');
    const resBLine = await waitConsoleAfter(c, /railsys17: resolve/, resBIdx, 30000);
    log('resolve branch: ' + resBLine);
    if (!resBLine.includes('-> rail-9') && !resBLine.includes('-> rail-')) {
      throw new Error('expected BRANCH resolve to spur rail: ' + resBLine);
    }

    // Switch network status.
    const stIdx = c.lines.length;
    await chat(c, 'railsys17 status');
    const swStatusLine = await waitConsoleAfter(c, /railsys17: junctions=/, stIdx, 30000);
    log('switch status: ' + swStatusLine);

    // Junction does NOT change the loop: rails3 status still prod=9 (8 loop +
    // 1 spur), loop total unchanged (222.84m for the 8 loop segments).
    ...
    if (!prodLine.includes('prod=9')) {
      throw new Error('expected prod=9 (8 loop + 1 spur): ' + prodLine);
    }

    // Screenshots: loop with the switch spur visible.
    const shots = [
      ['SS-R17-01_SWITCH_OVERVIEW.png', 'railsys3 camera 0 70 0 0 90'],
      ['SS-R17-02_SWITCH_PERSPECTIVE.png', 'railsys3 camera -25 24 45 40 28'],
      ['SS-R17-03_SWITCH_SPUR_NEAR.png', 'railsys3 camera -8 6 44 90 8'],
      ['SS-R17-04_LOOP_CURVE.png', 'railsys3 camera 18 6 16 225 10'],
      ['SS-R17-05_MODELPACK_LOOP.png', 'railsys3 camera 0 70 0 0 90'],
    ];
    ...
    log('build: ' + buildLine);
    log('spur: ' + spurLine);
    log('route THROUGH: ' + throughLine);
    log('resolve THROUGH: ' + resTLine);
    log('resolve BRANCH: ' + resBLine);
    log('switch status: ' + swStatusLine);
    log('prod status: ' + prodLine);
    log('JS_ERROR_COUNT=' + c.jsErrors);
    log('=== R17 ACCEPTANCE COMPLETE ===');
    return 0;
    log('=== SUMMARY ===');
    log('build: ' + buildLine);
    ...
    log('prod status: ' + prodLine);
    log('JS_ERROR_COUNT=' + c.jsErrors);
    log('=== R17 ACCEPTANCE ACCEPTANCE COMPLETE ===');
    return 0;
    return 0;
  } catch (e) {
    log('FATAL: ' + e.message);
