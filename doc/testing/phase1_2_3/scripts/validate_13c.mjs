#!/usr/bin/env node
/**
 * Phase 1.3C GUI validation: Rail Asset / ModelPack System.
 * Renders the same straight path with different assets (gauge/base/sleeper),
 * plus the fallback asset for an unknown id.
 */
...
const PROFILE = process.env.VALIDATION_PROFILE || resolve(PHASE, 'profiles/run-13c');
...
async function cmd(c, s) { await c.pressKey('t', 'KeyT', 84); await sleep(800); await c.typeLine(s); await sleep(300); await c.pressKey('Enter', 'Enter', 13); await sleep(2000); }
async function tp(c, x, y, z, yaw, pitch) { await c.pressKey('t', 'KeyT', 84); await sleep(800); await c.typeLine(`/tp ${x} ${y} ${z} ${yaw} ${pitch}`); await sleep(300); await c.pressKey('Enter', 'Enter', 13); await sleep(2500); }
...
async function main() {
  ...
  await cmd(c, '/railrender straight 40');
  await tp(c, 8, 84, 8, 0, -90);
  await sleep(2000);

  await cmd(c, '/railrender asset railsys.straight_1435_wood');
  await sleep(1500);
  await c.shot(RUN_ID + '_SS-P1_3C-01_asset_1435_wood.png');
  await cmd(c, '/railrender asset railsys.straight_1067_ballast');
  await sleep(1500);
  await c.shot(RUN_ID + '_SS-P1_3C-02_asset_1067_ballast.png');
  await cmd(c, '/railrender asset railsys.straight_1435_concrete');
  await sleep(1500);
  await c.shot(RUN_ID + '_SS-P1_3C-03_asset_1435_concrete.png');
  await cmd(c, '/railrender assetfallback');
  await sleep(1500);
  await c.shot(RUN_ID + '_SS-P1_3C-04_asset_fallback.png');
  await cmd(c, '/railrender asset railsys.straight_1435_wood');
  await sleep(1500);
  await c.shot(RUN_ID + '_SS-P1_3C-05_asset_restore_wood.png');
