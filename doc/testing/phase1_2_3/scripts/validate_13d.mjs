#!/usr/bin/env node
/**
 * Phase 1.3D GUI validation: performance on long rails.
 * Registers increasing-length paths and captures renderMs/segments via
 * /railrender status, plus screenshots at 500m for visual regression.
 */
...
const PROFILE = process.env.VALIDATION_PROFILE || resolve(PHASE, 'profiles/run-13d');
const PORT = process.env.CDP_PORT || '9451';
...
function launchChrome() {
  ...
  const child = spawn('/opt/google/chrome/chrome', args, {
    detached: true,
    stdio: ['ignore', 'pipe', 'pipe'],
  });
  child.stdout.on('data', (d) => {
    const s = d.toString();
    if (/RAILRENDER|RAILSYSTEM/.test(s)) process.stdout.write(s);
  });
  child.stderr.on('data', (d) => {
    const s = d.toString();
    if (/RAILRENDER|RAILSYSTEM/.test(s)) process.stderr.write(s);
  });
  child.unref();
}
...
async function measure(c, len) {
  await cmd(c, '/railrender clear');
  await cmd(c, `/railrender straight ${len}`);
  await sleep(2500);
  await cmd(c, '/railrender status');
  await sleep(1000);
  // capture the status chat line
  await c.shot(RUN_ID + `_SS-P1_3D-perf_${len}m_status.png`);
  await sleep(1500);
}
async function main() {
  launchChrome();
  ...
  log('=== perf: 500m ===');
  await measure(c, 500);
  await tp(c, 8, 90, 8, 0, -90);
  await sleep(3000);
  await c.shot(RUN_ID + '_SS-P1_3D-01_500m_overview.png');
  await cmd(c, '/railrender status');
  await sleep(2000);
  log('DONE');
  process.exit(0);
