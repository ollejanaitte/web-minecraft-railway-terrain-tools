...
const PROFILE = process.env.CHROME_PROFILE || resolve(PHASE, 'chrome-profile-cursor');
...
async function waitInWorld(cdp) {
  for (let i = 1; i <= 60; i++) {
    ...
    const m = analyze(p);
    log('join ' + i + ' ' + m);
    if (/Crash|WORLD_UNLOADING|fallen/i.test(m) || /Crash|WORLD_UNLOADING|fallen/i.test(cdp.lines.slice(-5).join(' '))) {
      // check image name via visual later; also console
    }
    const sky = parseFloat((m.match(/sky=([0-9.]+)/) || [])[1] || 0);
    ...
    if (validated || grounded) {
      log('IN_WORLD validated=' + validated + ' grounded=' + grounded);
...
async function main() {
  ...
  if (!ok) throw new Error('failed in-world');
