...
async function enterWorld(cdp) {
  ...
  await sleep(300);
  for (const ch of 'EaglerValidate') {
    await typeChar(ch);
    await sleep(40);
  }
  await sleep(1000);
  // PROBE candidate More-World-Options positions
  for (const y of [197, 225, 248, 153, 115]) {
    await cdp.shot(resolve(S, '_op' + y + '_before.png'));
    await cdp.click(640, y);
    await sleep(5000);
    await cdp.shot(resolve(S, '_op' + y + '_after.png'));
    log('CLICK y=' + y + ' -> ' + analyze(resolve(S, '_op' + y + '_after.png')));
  }
  await sleep(3000);
  return 'probed';
}
