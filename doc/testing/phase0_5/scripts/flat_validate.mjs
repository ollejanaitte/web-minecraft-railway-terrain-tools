...
const PROFILE = process.env.CHROME_PROFILE || resolve(PHASE, 'profiles/flat-run');
...
const LOG = resolve(PHASE, 'logs/flat_validate.log');
...
async function enterWorld(cdp) {
  ...
  await sleep(1000);
  // Phase 0.5: the world name marker "eaglerflat" forces the Superflat type in
  // GuiCreateWorld; no UI world-type navigation needed. Just create the world.
  await cdp.click(400, 540); // Confirm create
  await sleep(15000);
  await cdp.shot(resolve(S, '_flat_loading.png'));
  log('loading ' + analyze(resolve(S, '_flat_loading.png')));
  return 'created';
}
...
async function waitInWorld(cdp) {
  ...
  for (let i = 1; i <= 60; i++) {
    ...
    if (validated || grounded) {
      ...
      await cdp.shot(resolve(S, 'SS-FLAT-01b_IN_WORLD.png'));
...
async function captureSeries(cdp) {
  // Phase 0.5 flat-world camera-tour shots (SS-FLAT-xx).
  await sleep(1000);
  await cdp.shot(resolve(S, 'SS-FLAT-01_WORLD_BOOT.png'));
  log('ok SS-FLAT-01 ' + analyze(resolve(S, 'SS-FLAT-01_WORLD_BOOT.png')));
  await sleep(4000);
  await cdp.shot(resolve(S, 'SS-FLAT-02_FLAT_GROUND.png'));
  log('ok SS-FLAT-02 ' + analyze(resolve(S, 'SS-FLAT-02_FLAT_GROUND.png')));
  for (const [n, w] of [
    ['SS-FLAT-03_STRAIGHT_RAIL.png', 4000],
    ['SS-FLAT-04_CURVE_RAIL.png', 5000],
    ['SS-FLAT-05_FULL_SCALE_TRAIN.png', 7000],
    ['SS-FLAT-06_FORMATION.png', 7000],
    ['SS-FLAT-07_BOUNDARY.png', 8000],
  ]) {
    await sleep(w);
    try {
