...
const LOG = resolve(ROOT, 'doc/testing/phase0_5/logs/opt_test.log');
...
function launch(){const args=['--headless=new','--no-sandbox','--use-gl=angle','--use-angle=vulkan','--autoplay-policy=no-user-gesture-required','--mute-audio','--hide-scrollbars','--window-size=1280,720',`--remote-debugging-port=${PORT}`,`--user-data-dir=${PROFILE}`,'file://'+HTML];const cmd=`exec /opt/google/chrome/chrome ${args.map(a=>JSON.stringify(a)).join(' ')} >${JSON.stringify(resolve(ROOT,'doc/testing/phase0_5/logs/opt_chrome.log'))} 2>&1`;const ch=spawn('bash',['-c',cmd],{stdio:'ignore',detached:true});ch.unref();}
function analyze(path){return spawnSync('python3',[resolve(PHASE,'scripts/analyze_buttons.py'),path],{encoding:'utf8'}).stdout.trim();}
async function waitMenu(cdp){for(let i=0;i<90;i++){await cdp.click(640,360);await sleep(1500);if(i===0){await cdp.click(640,390);await sleep(700);await cdp.click(640,450);await sleep(700);}const p=resolve(S,'_ot_boot.png');await cdp.shot(p);const a=analyze(p);if(/menu-clouds|dirt-screen/.test(a)&&!/loading/.test(a)){log('boot '+i+' '+a);const bt=(a.match(/buttons=([\d,]+)/)||[])[1]||'';if(bt.split(',').length>=1)return bt.split(',').map(Number);}}throw new Error('no menu');}
async function main(){
  writeFileSync(LOG,'');launch();await sleep(6000);
  const cdp=new CDP(await getWs());await cdp.connect();await cdp.send('Runtime.enable');
  const menu=await waitMenu(cdp);log('MENU='+menu.join(','));
  await cdp.click(640,menu[0]);await sleep(10000);
  const p1=resolve(S,'_ot_sp.png');await cdp.shot(p1);const a1=analyze(p1);log('SP '+a1);
  const b1=(a1.match(/buttons=([\d,]+)/)||[])[1]||'';const y1=b1?Number(b1.split(',').pop()):492;
  await cdp.click(915,y1);await sleep(7000);
  const p2=resolve(S,'_ot_sub.png');await cdp.shot(p2);const a2=analyze(p2);log('SUB '+a2);
  const b2=(a2.match(/buttons=([\d,]+)/)||[])[1]||'';const ys2=b2.split(',').map(Number);
  const y2=ys2.length?ys2[0]:242;
  await cdp.click(640,y2);await sleep(7000);
  const p3=resolve(S,'_ot_form.png');await cdp.shot(p3);const a3=analyze(p3);log('FORM '+a3);
  await cdp.click(495,138);await sleep(600);
  ...
  for(const ch of 'FlatTest'){await tc(ch);await sleep(30);}
  await sleep(600);
  // click MoreOptions candidate y=392
  await cdp.click(640,392);await sleep(4000);
  const p4=resolve(S,'_ot_opts.png');await cdp.shot(p4);log('AFTER y392 '+analyze(p4));
  process.exit(0);
}
