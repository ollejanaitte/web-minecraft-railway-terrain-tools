#!/usr/bin/env bash
# Phase 1.1 geometry visual validation (Flat World + Hardware Vulkan).
set -uo pipefail
ROOT="$(cd "$(dirname "$0")" && pwd)"
cd "$ROOT" || exit 2
export JAVA_HOME="${JAVA_HOME:-/usr/lib/jvm/java-17-openjdk-amd64}"
PHASE="$ROOT/doc/testing/phase1_1"
LOGD="$PHASE/logs"
STAMP="$(date +%Y%m%d_%H%M%S)"
MAX_RUNS="${MAX_RUNS:-4}"
GPU_MODE="${GPU_MODE:-auto}"
CDP_PORT="${CDP_PORT:-9391}"
mkdir -p "$LOGD" "$PHASE/screenshots" "$PHASE/profiles"
say() { echo "[run-phase1_1-geom-visual] $*"; }
...
for p in $(pgrep -f "phase1_1/profiles" || true); do kill -9 "$p" 2>/dev/null || true; done
...
for ((run=1; run<=MAX_RUNS; run++)); do
  say "=== geom visual run $run/$MAX_RUNS ==="
  PROFILE="$PHASE/profiles/geom-$STAMP-$run"
  rm -rf "$PROFILE"
  if GPU_MODE="$GPU_MODE" CDP_PORT="$CDP_PORT" CHROME_PROFILE="$PROFILE" \
     node "$PHASE/scripts/geom_validate.mjs" >"$LOGD/geom_${STAMP}_${run}.log" 2>&1; then
    PASS=1
    say "run $run PASS"
    break
  fi
  say "run $run FAILED - retrying"
  for p in $(pgrep -f "geom-${STAMP}-${run}" || true); do kill -9 "$p" 2>/dev/null || true; done
  sleep 3
done

for p in $(pgrep -f "phase1_1/profiles/geom-" || true); do kill -9 "$p" 2>/dev/null || true; done
for p in $(pgrep -f "remote-debugging-port=${CDP_PORT}" || true); do kill -9 "$p" 2>/dev/null || true; done

if [ "$PASS" = "1" ]; then
  say "=== PHASE1.1 GEOM VISUAL PASS ==="
  echo "PHASE1_1_GEOM_VISUAL=PASS"
  exit 0
else
  say "=== PHASE1.1 GEOM VISUAL FAIL ==="
  echo "PHASE1_1_GEOM_VISUAL=FAIL"
  exit 1
fi
